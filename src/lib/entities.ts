...
export const ENTITIES: Record<EntityKind, EntityDef> = {
  event: { noun: 'event', href: (id) => `/event/${q(id)}`, isCode: true },
  session: { noun: 'session', href: (id) => `/sessions/${q(id)}`, events: (id) => history(`session:${id}`), isCode: true },
  source: { noun: 'source IP', href: (id) => `/investigate/ip/${q(id)}`, events: (id) => `/events?ip=${q(id)}` },
  ...
  payload: { noun: 'payload', href: (id) => `/payload-analysis/${q(id)}`, isCode: true },
