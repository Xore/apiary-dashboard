...
import { ContainerStateLabel } from '#/components/FeedState'
...
function SettingsPage() {
  ...
  const { pane = 'account' } = Route.useSearch()
  ...
  // Leaving with unsaved personal preferences asks first.
  useBlocker({ shouldBlockFn: () => dirty && !window.confirm('Discard unsaved preference changes?'), enableBeforeUnload: () => dirty })
        ...
        return ids.length ? (
          <VStack key={group} gap={1}>
            ...
            <List density="compact">
              {ids.map((id) => (
                <ListItem key={id} label={PANES[id].title} isSelected={id === pane} onClick={() => void navigate({ search: { pane: id === 'account' ? undefined : id } })} />
              ))}
            </List>
          </VStack>
        ) : null
      ...
      header={
        <LayoutHeader>
          <VStack gap={1}>
            ...
            <Text color="secondary">Personal preferences, and platform configuration for admins.</Text>
          </VStack>
        </LayoutHeader>
      }
      start={
        isNarrow ? undefined : (
          <LayoutPanel width={260} hasDivider label="Settings sections">
            {rail}
          </LayoutPanel>
        )
      }
